...
// Capacidade usada em algum ponto para JSON
const size_t CAPACITY = JSON_ARRAY_SIZE(12);

// Construtor padrão da classe HTTPService
HTTPService::HTTPService() {}

// Obtém informações do hardware
// O resultado é preenchido por referência em `record`
void HTTPService::getInfoHardware(HardwareRecord &record){

    // hardware/MAC?api_token = TOKEN_BASE&tipo-hardware=TIPO

    Hardware hardware;    // Representa o proprio ESP-32
    
    // Variáveis usadas para montar url da requisicao
    String route;         // Rota base da API
    String routeService;
    String type = "GET";
    String params = "";
    ...
    // Seleção de rota com base na configuração
    ...
    // Validação básica da resposta (strings de erro padronizadas)
    if (strstr(response.c_str(), "[ERROR]") == NULL && strstr(response.c_str(), "[NO_CONTENT]") == NULL){
         // Documento JSON alocado na stack (buffer interno no heap)
        DynamicJsonDocument doc(2048);

         // Tentativa de parse do JSON
        DeserializationError error = deserializeJson(doc, response);
        // Erro na deserializacao json
        if (error){
            // Logs condicionais (debug)
            if (config.isDebug())
                Serial.println(String("[HTTPService::getInfoHardware()] Falha no parse JSON.") + error.f_str());

            //Delay de 5 segundos antes de abortar.
            delay(5000);
            return;
        }

        // Se código da rsp http é 200, sucesso
        if (doc["httpCode"].as<int>() == 200){
            ...
            // Caso contrario erro na rsp http.
            if (config.isDebug())
                Serial.print(String("[HTTPService::getInfoHardware()] Mensagem: ") + doc["message"].as<const char *>());
            return;
        }
    }
    else
    ...
    // Em caso de erro ou NO_CONTENT, apenas retorna
    return;
}
